...
test('only a generated header establishes ownership; quoting the marker is not ownership', () => {
  const { isGenerated } = require('./sync-adapters');
  const marker='Generated by scripts/sync-adapters.js; edit the canonical .claude source.';
  assert.equal(isGenerated(`Custom documentation mentions ${marker}`),false);
  assert.equal(isGenerated(`// ${marker}\n"use strict";\n`),true);
  assert.equal(isGenerated(`---\nname: debug\n---\n\n<!-- ${marker} -->\n`),true);
  assert.equal(isGenerated(`# Custom note\n\n<!-- ${marker} -->\n`),false);
});
