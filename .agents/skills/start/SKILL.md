...
project`, which retrofits an existing codebase. If `.Codex/context/project-context.md` is
filled or a root `AGENTS.md` exists, stop and say so.
...
`.Codex/templates/docs/decision.md`: context (the request and its constraints), decision
...
Create the minimal skeleton for the chosen type from `.Codex/reference/architectures.md`.
...
- Set `testGate.command` and `format.command` in `.Codex/conductor.config.json`.
...
- Generate the root `AGENTS.md` from `.Codex/templates/Codex-root.md`, keeping
  `@.Codex/AGENTS.md` as the first line. Philosophy, conventions, and domain rules only —
  never a directory listing or a dependency list, which Codex derives from the code.
- Fill `.Codex/context/project-context.md`: purpose, stack, commands, architecture, risks.
- Copy the matching pack from `.Codex/templates/stacks/` to
  `.Codex/skills/<stack>/SKILL.md`. If no pack matches, say so — do not invent one.
- Check `.Codex/rules/`: the shipped rules are path-scoped and activate on their own, but
